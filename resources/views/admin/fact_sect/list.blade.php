      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">FACT_SECT &nbsp;<a href="{{ asset('admin/fact_sect/add') }}" class="btn btn-warning btn-sm"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add Data</a></h3>
          ...
          <div class="box-body table-responsive no-padding">
            <table class="table table-hover form-delete" data-toggle="dataTable" data-form="deleteForm">
              ...
              {{-- --------- SECTION LIST ---------- --}}
              @foreach($factory_section as $fact_sect)
              <?php
              $stt++;
              ?>
              <tr>
                <td>{{ $stt }}</td>
                <td>{{ $fact_sect -> Fact_Name }}</td>
                <td>{{ $fact_sect -> Sect_Name }}</td>
                <td class="pull-right">
                  <a href="{{ asset('admin/fact_sect/edit').'/'.$fact_sect-> Fact_ID.'/'.$fact_sect-> Sect_ID}}" class="btn btn-success"><i class="fa fa-pencil"></i> Edit</a>
                  <button data-toggle="modal" data-target="#confirm-delete" data-href="{{ asset('admin/fact_sect/delete').'/'.$fact_sect-> Fact_ID.'/'.$fact_sect-> Sect_ID }}" class="btn btn-danger" id="delete-btn"><i class="fa fa-trash"></i> Delete</a>
                  </td>
                </tr>
                @endforeach
                {{-- --------- END SECTION LIST ---------- --}}
