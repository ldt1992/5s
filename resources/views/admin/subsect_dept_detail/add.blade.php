@extends('admin.master')


@section('css')
<link rel="stylesheet" href="{{ asset('public/admin_asset/bootstrap/css/bootstrap.min.css') }}">
<!-- Font Awesome -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.5.0/css/font-awesome.min.css">
<!-- Ionicons -->
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/ionicons/2.0.1/css/ionicons.min.css">
<!-- Theme style -->
<link rel="stylesheet" href="{{ asset('public/admin_asset/dist/css/AdminLTE.min.css') }}">
  <!-- AdminLTE Skins. Choose a skin from the css/skins
  folder instead of downloading all of them to reduce the load. -->
  <link rel="stylesheet" href="{{ asset('public/admin_asset/dist/css/skins/_all-skins.min.css') }}">
  @endsection


  @section('breadcrumbs')
  <!-- Content Header (Page header) -->
  <section class="content-header">
    <h1>
      Subsection - Department - Detail
      <small>Version 2.0</small>
    </h1>
    <ol class="breadcrumb">
      <li><a href="{{ asset('admin') }}"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class="active">Subsect_Dept_Details</li>
    </ol>
  </section>
  @endsection


  @section('content')
  <!-- Main content -->
  <section class="content">
    <div class="row">
      <!-- right column -->
      <div class="col-md-4">

        <!-- Horizontal Form -->
        <div class="box box-info">
          <div class="box-header with-border">
            <h3 class="box-title">Thêm nhiều dữ liệu cùng lúc</h3>
          </div>
          <!-- /.box-header -->

          <!-- form start -->
          <form class="form-horizontal">
            <div class="box-body">
              <div class="form-group">
                <label class="col-sm-2 control-label">Chọn số lượng</label>

                <div class="col-sm-10">
                  <select class="form-control" name="selSoLuong" id="selSoLuong">
                    @for ($i = 1; $i <= 20 ; $i++)
                      <option value="{{ $i }}">{{ $i }}</option>
                    @endfor
                  </select>
                </div>
              </div>
            </div>
          </form>
        </div>
        <!-- /.box -->
        
      </div>
      <div class="col-md-8">
        {{-- -------- Add New SubSection route('postAddSubsect') ----------- --}}
        <form action="{{ route('postAddSubsect_Dept_Detail') }}" method="post">
          <input type="hidden" name="_token" value="{{ csrf_token() }}">
          <!-- Horizontal Form -->
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Add New Subsection - Department - Details</h3>
            </div>
            <!-- /.box-header -->

            {{-- ------ Validation ------ --}}
            @if (count($errors) > 0)
            <div class="box-body">
              <div class="alert alert-danger">
                <h4><i class="icon fa fa-ban"></i>
                  Errors !
                </h4>
                <ul>
                  @foreach ($errors->all() as $error)
                  <li>{{ $error }}</li>
                  @endforeach
                </ul>
              </div>
            </div>
            @endif
            {{-- ------ End Validation ------ --}}

            <!-- form start -->
            <form class="form-horizontal">
              <div class="box-body" id="them">
                <div class="form-group">
                  <label class="col-sm-2 control-label">Subsection</label>

                  <div class="col-sm-10">
                    <select class="form-control" name="selSubsection">
                      {{-- ------ subsections ------- --}}
                      @foreach ($subsections as $subsect)
                      <option value="{{ $subsect-> id }}">{{ $subsect-> Subsect_Name }}</option>
                      @endforeach
                      {{-- ------ end subsections ------- --}}
                    </select>
                  </div>
                </div>
                <hr>
                <div class="form-group">
                  <label class="col-sm-2 control-label">Department</label>

                  <div class="col-sm-10">
                    <select class="form-control" name="selDepartment">
                      {{-- ------ Department ------- --}}
                      @foreach ($departments as $dept)
                      <option value="{{ $dept-> id }}">{{ $dept-> Dept_Name }}</option>
                      @endforeach
                      {{-- ------ end Department ------- --}}
                    </select>
                  </div>
                </div>
                <hr>
                <hr>
                <div class="form-group">
                  <label class="col-sm-2 control-label">Details</label>

                  <div class="col-sm-10">
                    <select class="form-control" name="selDetail">
                      {{-- ------ Details ------- --}}
                      @foreach ($details as $dt)
                      <option value="{{ $dt-> id }}">{{ $dt-> Detail_Content }}</option>
                      @endforeach
                      {{-- ------ end Details ------- --}}
                    </select>
                  </div>
                </div>
              </div>
              <div class="box-footer">
                <a href="{{ asset('admin/subsect_dept_detail/list') }}" class="btn btn-default"><i class="fa fa-chevron-left"></i> Back</a>
                <button type="submit" class="btn btn-success pull-right">Save</button>
              </div>
              <!-- /.box-footer -->
            </form>
          </div>
          <!-- /.box -->
        </form>
        <hr>
        {{-- -------- End New SubSection ----------- --}}
        <!-- /.box -->
      </div>
      <!--/.col (right) -->
    </div>
    <!-- /.row -->
  </section>
  <!-- /.content -->
  @endsection


  @section('script')
  <!-- jQuery 2.2.3 -->
  <script src="{{ asset('public/admin_asset/plugins/jQuery/jquery-2.2.3.min.js') }}"></script>
  <!-- Bootstrap 3.3.6 -->
  <script src="{{ asset('public/admin_asset/bootstrap/js/bootstrap.min.js') }}"></script>
  <!-- FastClick -->
  <script src="{{ asset('public/admin_asset/plugins/fastclick/fastclick.js') }}"></script>
  <!-- AdminLTE App -->
  <script src="{{ asset('public/admin_asset/dist/js/app.min.js') }}"></script>
  <!-- AdminLTE for demo purposes -->
  <script src="{{ asset('public/admin_asset/dist/js/demo.js') }}"></script>
  @endsection